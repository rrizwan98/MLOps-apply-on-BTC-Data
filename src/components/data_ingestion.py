...
# # train_data_len1 = data[:train_data_len]
...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            df=pd.read_csv('Dataset\BTC-USD 2020 - 2023.csv')
            ...
            # filter the first 85% data for training and 15% for testing
            data = df.filter(['Close'])
            dataset = data.values
            train_data_len = math.ceil(len(dataset)*.85)
            train_set = data[:train_data_len]
